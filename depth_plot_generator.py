...
constant_depth = np.loadtxt('C:/Users/willi/PycharmProjects/pythonProject/URECA-RL-Based-3D-Printing/'
                            'constant_depth_frameskip.txt') * 1000
inconsistent_depth = np.loadtxt('C:/Users/willi/PycharmProjects/pythonProject/URECA-RL-Based-3D-Printing/'
                                'inconsistent_depth_frameskip.txt') * 1000
...
plt.plot(times, constant_depth, label='Constant Velocity Depth', color='blue', linewidth=2.0)
plt.plot(times, inconsistent_depth, label='Inconsistent Velocity Depth', color='orange', linewidth=2.0)

plt.xlim(left=0)  # Set the x-axis to start from the minimum time value
plt.ylim(-1.3, 0)       # Set the y-axis to start from the minimum depth value

font_size = 14
plt.xlabel('Time, t [s]', fontsize=20)
plt.ylabel('Melt Pool Depth, d [mm]', fontsize=20)
# plt.title('Comparison of Melt Pool Depth Profiles', fontsize=font_size)

plt.legend(loc='lower right', fontsize=font_size)

fig_dir = '/path/to/save/directory'  # Replace with the actual directory path
